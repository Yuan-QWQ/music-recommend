...
def like_comment(request, comment_id, movie_id):
    ...
    exit_comment = LikeComment.objects.filter(user_id=user_id, comment_id=comment_id)
    if exit_comment.count() == 0:
        LikeComment.objects.create(user_id=user_id, comment_id=comment_id)
    else:
        exit_comment.delete()
