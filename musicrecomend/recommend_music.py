# -*-coding:utf-8-*-
import os#对文件，文件夹执行操作

os.environ["DJANGO_SETTINGS_MODULE"] = "musicrecomend.settings"
import django

django.setup()
from music.models import *#数据库建表
from math import sqrt, pow#数学函数
import operator#实现比较功能
...
class UserCf:
    ...
    def pearson(self, user1, user2):  #
        ...
        for movie1, score1 in user1.items():
            if movie1 in user2.keys():  # 计算公共的浏览次数
                n += 1
                sum_xy += score1 * user2[movie1]#user1和user2对movie1的评分乘积和
                sum_x += score1#user1对各音乐的评分总和
                sum_y += user2[movie1]##user2对各音乐的评分总和
                sumX2 += pow(score1, 2)
    ...
    def nearest_user(self, current_user, n=1):
        ...
        for user, rate_set in self.all_user.items():
            ...
            if user != current_user:
                distance = self.pearson(self.all_user[current_user], self.all_user[user])#计算当前用户与其它用户的皮尔逊系数
                # 计算两个用户的相似度
    ...
    def recommend(self, username, n=3):
        ...
        for user, score in dict(nearest_user).items():  # 最相近的n个用户 user为最相近用户的名字 score为与当前用户的距离
            for movies, scores in self.all_user[user].items():  # 推荐的用户的音乐列表 音乐名和评分
                if movies not in self.all_user[username].keys():  # 当前username没有看过
                    if movies not in recommend.keys():  # 添加到推荐列表中
                        recommend[movies] = scores*score#为用户推荐的用户为该音乐的评分*推荐用户与当前用户的相似度(距离)
        # 对推荐的结果按照音乐
        # 浏览次数排序
...
def recommend_by_user_id(user_id):
    user_prefer = UserTagPrefer.objects.filter(user_id=user_id).order_by('-score').values_list('tag_id', flat=True)#从数据库中获取数据 返回列表
...
def recommend_by_item_id(user_id, k=15):
    ...
    # 选用户最喜欢的标签中的音乐，用户没听过的30首，对这30首音乐，计算距离最近
    un_watched = Movie.objects.filter(~Q(rate__user_id=user_id), tags__in=user_prefer).order_by('-num')[:30]  # 没听过的音乐
    watched = Rate.objects.filter(user_id=user_id).values_list('movie_id', 'mark')
    print(f"un_watched:{un_watched}")
